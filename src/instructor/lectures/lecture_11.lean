...
def le (n m: ℕ) : Prop := n - m <= 0
#reduce le 5 6 



def pyth_triple (n m p: ℕ): Prop := n*n + m*m = p*p

example: ∃ (a b c: ℕ), pyth_triple a b c:= 
begin 
  unfold pyth_triple, 

end 

example: pyth_triple 3 4 5:=
begin
  unfold pyth_triple, 
  apply eq.refl, 
end 

example: pyth_triple 4 5 6:=
begin
  unfold pyth_triple, 
  apply eq.refl, 
end 

def red_blue: ∃ (n: ℕ), red n ∧ blue n → 
∃  (n: ℕ ), red n 
/- 
