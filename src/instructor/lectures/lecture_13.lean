...
begin
  ...
  cases pf, 
  apply pf, 
  --can't prove because we are not sure the ball is red 
end 
...
axioms
  (Person : Type)
  (Nice : Person → Prop)
  --takes two people and returns prop that person 1 likes person 2
...
example: ∃ (p1 : Person, ∀ p2 : Person, Likes p2 p1) → 
  ...
  begin 
    ...
    assume e, 
    apply exists.intro_ _,
    exact (w e), 
  end 

  --If there's someone everyone likes, everyone likes someone 
  --any arbitrary person p2 likes someone p1 
/- begin
  assume h, 
  assume e, 
  --need to show that there exsists a person that e likes 
  --what is this "with" thing? p implies a person pf a proof 
  cases h with p pf, 
